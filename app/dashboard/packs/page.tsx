...
import { SetPickerCard } from '@/components/packs/set-picker-card'
import type { DashboardStats } from '@/lib/economy/types'
import type { SetProgress, SetSummary, SetWithProgress } from '@/lib/packs/types'
...
export default async function PacksPage() {
  ...
  const progressBySetId = new Map(
    ((progress ?? []) as SetProgress[]).map((row) => [row.set_id, row]),
  )

  const setsWithProgress: SetWithProgress[] = (sets as SetSummary[]).map((set) => {
    ...
    <main className="min-h-screen">
      ...
      <div className="p-6">
        <div className="mb-6 flex items-center justify-between">
          <h1 className="text-xl font-bold">Buka Pack</h1>
